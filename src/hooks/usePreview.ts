...
/**
 * Preview engine that supports conditional flow and all question types.
 *
 * Answer stores:
 *  - answers: Record<guid, string[]>          → SingleChoice / MultipleChoice
 *  - textAnswers: Record<guid, string>        → TextEntry
 ...
 */
export function usePreview(questions: Question[], conditions: ConditionalRule[] = [], sequentialEdges?: SequentialEdges) {
   ...
   * This mirrors exactly how FlowCanvas buildEdges works.
   */
  function buildFlowMap(): Map<string, string> {
    ...
    // 1. Default sequential edges by order (only if not blocked)
    ...
    // 2. Custom edges OVERRIDE default mappings
   ...
   * Also checks sequential edges for custom flow paths.
   */
  function resolveNext(
    ...
    // 2. Check sequential flow map (built from both default and custom edges)
    ...
    // (This happens when all edges from this question are blocked)
    if (sequentialEdges && (sequentialEdges.blockedEdges?.length || sequentialEdges.customEdges?.length)) {
      return { type: 'end' };
    }

    // 4. Default sequential flow (by order) - only when no sequential edges are configured
  ...
  function checkControlQuestion(question: Question): void {
    ...
    switch (question.type) {
      ...
      case 7: // Rating
        ...
        break;
      default:
        return; // Control questions only supported for Choice and Rating
    }

    // Compare answers (order doesn't matter for arrays)
    const isCorrect =
  ...
  const goNext = useCallback(() => {
    ...
    if (result.type === 'question') {
      ...
      if (targetExists) {
        ...
        // Target doesn't exist, fall through to sequential
    ...
    // Sequential fallback: use sorted questions (by order) to determine next question
    const currentIdx = sortedQuestions.findIndex((q) => q.guid === currentGuid);
    if (currentIdx < sortedQuestions.length - 1) {
      setPath((prev) => [...prev, sortedQuestions[currentIdx + 1].guid]);
    } else {
